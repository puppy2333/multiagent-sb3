...
class Monitor(gym.Wrapper[ObsType, ActType, ObsType, ActType]):
    ...
    def reset(self, **kwargs) -> Tuple[ObsType, Dict[str, Any]]:
        ...
        curr_agent = (self.curr_agent + self.n_agents - 1) % self.n_agents
        self.rewards_list[curr_agent] = []
    ...
    def step(self, action: ActType) -> Tuple[ObsType, SupportsFloat, bool, bool, Dict[str, Any]]:
        ...
        self.rewards_list[self.curr_agent].append(float(reward))
        
        if terminated or truncated:
            self.needs_reset = True
            
            ep_rew = sum(self.rewards_list[self.curr_agent])
            ep_len = len(self.rewards_list[self.curr_agent])
            ep_info = {
                "r" + str(self.curr_agent): round(ep_rew, 6), 
                "l" + str(self.curr_agent): ep_len, 
                "t": round(time.time() - self.t_start, 6)}
            
            for key in self.info_keywords:
                ep_info[key] = info[key]

            # episode_returns... are never called, no need to update
        ...
        self.curr_agent = (self.curr_agent + 1) % self.n_agents
